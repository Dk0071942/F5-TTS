...
def find_models_in_ckpts():
    """Search for models and vocab files in the ckpts directory."""
    ckpts_dir = Path(__file__).parent.parent.parent.parent / "ckpts"
    ...
    for root, dirs, files in os.walk(ckpts_dir):
        for file in files:
            if file.endswith(".safetensors"):
                model_paths.append(str(Path(root) / file))
            elif file == "vocab.txt":
                vocab_paths.append(str(Path(root) / file))
...
"""
    ...
    def scan_local_models():
        """Scan the ckpts directory for model checkpoints and vocab files."""
        ...
        for root, _, files in os.walk(ckpts_dir):
            for file in files:
                if file.endswith(".safetensors"):
                    model_files.append(str(Path(root) / file))
                elif file == "vocab.txt":
                    vocab_files.append(str(Path(root) / file))

        return model_files, vocab_files
    ...
    # Get local models and vocab files
    local_model_files, local_vocab_files = scan_local_models()
    ...
    def switch_tts_model(new_choice):
        ...
        if new_choice == "Custom":  # override in case webpage is refreshed
            ...
            return (
                gr.update(
                    visible=True,
                    value=custom_ckpt_path,
                    choices=local_model_files + [DEFAULT_TTS_MODEL_CFG[0]],
                ),
                gr.update(
                    visible=True,
                    value=custom_vocab_path,
                    choices=local_vocab_files + [DEFAULT_TTS_MODEL_CFG[1]],
                ),
    ...
    # Find available models and vocab files
    available_models, available_vocabs = find_models_in_ckpts()

    # Update the custom model dropdowns with found files
    ...
    custom_vocab_path = gr.Dropdown(
        ...
        value=available_vocabs[0] if available_vocabs else "",
        allow_custom_value=True,
